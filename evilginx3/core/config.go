...
const DEFAULT_UNAUTH_URL = "https://example.com"
...
func (c *Config) SetTelegramSettings(botToken string, chatID string) error {
	c.Telegram.BotToken = botToken
	c.Telegram.ChatID = chatID
	return c.cfg.WriteConfig()
}

func (c *Config) GetTelegramSettings() (string, string) {
	return c.Telegram.BotToken, c.Telegram.ChatID
}

func (c *Config) initTelegram() {
	c.telegram.bot_token = c.cfg.GetString("telegram.bot_token")
	c.telegram.chat_id = c.cfg.GetString("telegram.chat_id")
}

func (c *Config) SetTelegramBotToken(token string) {
	c.general.TelegramBotToken = token
	// Save all general settings
	c.cfg.Set("general", map[string]interface{}{
		"domain":            c.general.Domain,
		"ipv4":             c.general.OldIpv4,
		"external_ipv4":    c.general.ExternalIpv4,
		"bind_ipv4":        c.general.BindIpv4,
		"unauth_url":       c.general.UnauthUrl,
		"https_port":       c.general.HttpsPort,
		"dns_port":         c.general.DnsPort,
		"autocert":         c.general.Autocert,
		"telegram_bot_token": token,
		"telegram_chat_id": c.general.TelegramChatID,
	})
	c.cfg.WriteConfig()
}
...
func (c *Config) SetTelegramChatID(chatID string) {
	...
	c.cfg.Set("general", map[string]interface{}{
		...
		"dns_port":         c.general.DnsPort,
		"autocert":         c.general.Autocert,
		"telegram_bot_token": c.general.TelegramBotToken,
