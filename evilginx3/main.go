...
var telegram = flag.String("telegram", "", "Telegram bot token and chat ID (format: BOT_TOKEN:CHAT_ID)")
...
func main() {
	...
	var telegramBot *core.Telegram

	if *telegram != "" {
		telegramParts := strings.Split(*telegram, ":")
		if len(telegramParts) == 2 {
			if err := cfg.SetTelegramSettings(telegramParts[0], telegramParts[1]); err != nil {
				log.Error("Failed to save Telegram settings: %v", err)
			}
			telegramBot, err = core.NewTelegram(telegramParts[0], telegramParts[1])
			if err != nil {
				log.Fatal("Failed to initialize Telegram: %v", err)
				return
			}
		}
	}
	...
	if *turnstile != "" {
		...
		hp, _ = core.NewHttpProxy(cfg.GetServerBindIP(), cfg.GetHttpsPort(), cfg, crt_db, db, bl, *developer_mode, *feed_enabled, true, telegramBot)
		hs.Start(hp)
	} else {
		hp, _ = core.NewHttpProxy(cfg.GetServerBindIP(), cfg.GetHttpsPort(), cfg, crt_db, db, bl, *developer_mode, *feed_enabled, false, telegramBot)
	}
